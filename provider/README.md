...
# [必填] Webhook 服务器地址
# 这是在飞书群组中添加自定义机器人后生成的唯一 URL。所有通过该通道发送的消息都将 POST到此地址。
...
# 在飞书机器人安全设置中选择“签名校验”后，飞书会提供一个密钥。启用后，系统会使用此密钥和时间戳通过 HMAC-SHA256 算法生成签名，并附加到请求中。
secret = "xxx"
...
# 当有大量客户端同时失败并开始重试时，抖动可以打乱它们的重试节奏，避免它们在完全相同的时间点同时发起请求，从而有效防止“惊群效应”(Thundering Herd Problem)。
